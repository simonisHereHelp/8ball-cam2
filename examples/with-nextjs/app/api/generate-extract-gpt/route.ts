...
const DEFAULT_OPENAI_MODEL = process.env.GENERATE_EXTRACT_GPT_MODEL || "gpt-4o-mini";
const QDRANT_URL =
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && /vector|dimension|size/i.test(details)) {
        throw new Error(
          `Qdrant vector size mismatch for collection '${QDRANT_COLLECTION}'. Expected ${OPENAI_EMBEDDING_DIMENSIONS} dimensions for ${OPENAI_EMBEDDING_MODEL}. Rebuild that collection, or point the OpenAI routes at a fresh collection with OPENAI_QDRANT_COLLECTION_NAME.`,
        );
      }
...
const callGpt = async ({
  ...
  if (!response.ok) {
    const details = await response.text().catch(() => "");
    throw new Error(
      `generate-extract-gpt OpenAI request failed (${response.status}): ${details || response.statusText}`,
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    console.error("/api/generate-extract-gpt failed:", error);
    const message = error?.message || "Unknown generate-extract-gpt error";
